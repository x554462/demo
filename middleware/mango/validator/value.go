...
func (v Value) Int8() int8 {
	...
	if math.MinInt8 > x || math.MaxInt8 < x {
		exception.ThrowMsg(fmt.Sprintf("%s超出值范围", v.name), excode.ValidateError)
	}
	return int8(x)
}
...
func (v Value) Int64() int64 {
	...
	if err != nil {
		exception.ThrowMsg(v.name+" 非合法数字", excode.ValidateError)
	}
	if !v.noValidate {
		if v.min != nil && i < *v.min {
			exception.ThrowMsg(fmt.Sprintf("%s不能小于%d", v.name, *v.min), excode.ValidateError)
		}
		if v.max != nil && i > *v.max {
			exception.ThrowMsg(fmt.Sprintf("%s不能大于%d", v.name, *v.max), excode.ValidateError)
		}
	}
	return i
}
...
func (v Value) String() string {
	...
	if !v.noValidate {
		if re.MatchString(v.data) {
			exception.ThrowMsg(fmt.Sprintf("%s不能包含特殊字符%s", v.name, specialChar), excode.ValidateError)
		}
		xl := int64(l)
		if v.min != nil && xl < *v.min {
			exception.ThrowMsg(fmt.Sprintf("%s长度不能小于%d", v.name, *v.min), excode.ValidateError)
		}
		if v.max != nil && xl > *v.max {
			exception.ThrowMsg(fmt.Sprintf("%s长度不能大于%d", v.name, *v.min), excode.ValidateError)
		}
	}
	return v.data
}
...
func (n Nil) Int() int {
	if n.must {
		exception.ThrowMsg(fmt.Sprintf("%s不能为空", n.name), excode.ValidateError)
	}
	return 0
}
