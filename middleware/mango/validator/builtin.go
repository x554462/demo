...
func integer(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s必须为整数(%d~%d)", key, min, max), excode.ValidateError)
	var number int64
	...
	case string:
		...
		if number, parseErr = strconv.ParseInt(integer, 10, 64); parseErr != nil {
			return exception.New(parseErr.Error(), excode.ValidateError)
		}
	default:
		return err
	}
...
func varchar(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s必须为字符串，长度为%d~%d", key, min, max), excode.ValidateError)
	if !ok {
		return err
	}
...
func phone(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s错误，请填写正确的手机号", key), excode.ValidateError)
	if !ok {
		return err
	}
...
func numberchar(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s必须为数字字符，长度为%d~%d", key, min, max), excode.ValidateError)
	if !ok {
		return err
	}
	var counter int
...
func bankcard(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s错误，请填写正确的银行卡号", key), excode.ValidateError)
	if !ok {
		return err
	}
...
func date(v interface{}, key string, params []string) error {
	...
	err := exception.New(fmt.Sprintf("%s格式错误", key), excode.ValidateError)
	if !ok {
		return err
	}
