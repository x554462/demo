...
var (
	ErrUnsupported    = exception.New("unsupported type", excode.ValidateError)
	ErrUnknownTag     = exception.New("unknown tag", excode.ValidateError)
	ErrCannotValidate = exception.New("cannot validate unexported struct", excode.ValidateError)
)
...
func (mv *Validator) SetValidationFunc(name string, vf ValidationFunc) error {
	if name == "" {
		return exception.New("name cannot be empty", excode.RuntimeError)
	}
	if vf == nil {
...
func Validate(v interface{}) {
	err := defaultValidator.ValidateStruct(v)
